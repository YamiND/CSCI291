...
{
	if (isset($_POST['confirmAll']) && !empty($_POST['confirmAll']))
	{
		outputAllData($mysqli);
	}
}

function outputAllData($mysqli)
{
	...
	array_map('unlink', glob("allOutput*")); 
		...
		if ($stmt = $mysqli->prepare("SELECT students.studentID, studentClassList.courseID FROM students, studentClassList WHERE studentClassList.studentID = students.studentID"))
		{
			if ($stmt->execute())
			{
		...
		$outputFile = basename("allStudentOutput-$day.zip");
